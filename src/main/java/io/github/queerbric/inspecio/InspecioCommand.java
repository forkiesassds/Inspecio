	...
	public static void register(CommandDispatcher<FabricClientCommandSource> dispatcher, CommandRegistryAccess buildContext) {
...

		{
			literalSubCommand.then(literal("reload")
					...
					.then(literal("crafter")
							.executes(onGetter("containers/crafter", getter(cfg -> cfg.getContainersConfig().isCrafterEnabled())))
							.then(argument("value", BoolArgumentType.bool())
									.executes(onBooleanSetter("containers/crafter", setter((cfg, val) -> cfg.getContainersConfig().setCrafter(val))))))
